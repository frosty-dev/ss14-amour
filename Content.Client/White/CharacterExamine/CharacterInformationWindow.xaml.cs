...
public sealed partial class CharacterInformationWindow : FancyWindow
{
    ...
    // ReSharper disable once InconsistentNaming
    private RichTextLabel _rolePlayInfo => RoleplayInfo;
    ...
    public void UpdateUi(EntityUid examined, string? name = null, string? job = null, string? flavorText = null,
        Dictionary<RoleplayThinkPrototype,RoleplaySelection>? roleplaySelections = null)
    {
        ResetUi();
        ...
        if (roleplaySelections != null)
        {
            var formattedMessage = new FormattedMessage();
            foreach (var (proto,pref) in roleplaySelections)
            {
                formattedMessage.AddMarkup($"[bold]{Loc.GetString(proto.Name)}[/bold] - {Loc.GetString("roleplay-" + pref.ToString().ToLower())}\n");
            }
            _rolePlayInfo.SetMessage(formattedMessage);
        }
    }
